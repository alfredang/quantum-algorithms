...
def create_phase_flip_code_circuit(initial_state='0', error_qubit=None):
    """
    Create the complete 3-qubit phase-flip error correction circuit.
    ...
    Args:
        ...
        error_qubit: Which qubit to apply Z error to (0, 1, 2, or None for no error)

    Returns:
        QuantumCircuit for phase-flip error correction
    """
    ...
    # Step 2: Encode for phase-flip code
    # |0⟩ → |+⟩ → |+++⟩
    # |1⟩ → |-⟩ → |---⟩
    qc.h(0)
    qc.cx(0, 1)
    ...
    # Step 3: Introduce phase error (Z gate)
    if error_qubit is not None and 0 <= error_qubit <= 2:
        qc.z(error_qubit)
    qc.barrier()

    # Step 4: Syndrome measurement in X basis
    # Transform to computational basis
    qc.h(0)
    qc.h(1)
    qc.h(2)

    # Measure parities into ancilla qubits
...
def run_phase_flip_code(initial_state='0', error_qubit=1, shots=1024):
    """
    Run the 3-qubit phase-flip code on IBM Quantum hardware.

    Args:
        initial_state: '0' or '1' for logical qubit
        error_qubit: Which qubit to apply Z error (0, 1, 2, or None)
    ...
    # Create circuit
    qc = create_phase_flip_code_circuit(initial_state, error_qubit)

    error_str = f"qubit {error_qubit}" if error_qubit is not None else "none"

    print(f"\nCode type: 3-qubit phase-flip code")
    print(f"Initial logical state: |{initial_state}⟩")
    print(f"Error introduced: Z (phase-flip) on {error_str}")
...
def apply_correction(data_bits, syndrome):
    ...
    For phase-flip code, the syndrome indicates which qubit had a Z error.
    After syndrome measurement in X basis:
    - 00: No error
    - 01: Z error on qubit 2
    - 10: Z error on qubit 0
    - 11: Z error on qubit 1
    ...
    # For phase-flip code measured in X basis, Z errors appear as bit flips
    if syndrome == '10':
        corrected[0] = 1 - corrected[0]  # Correct qubit 0
    elif syndrome == '01':
        corrected[2] = 1 - corrected[2]  # Correct qubit 2
    elif syndrome == '11':
        corrected[1] = 1 - corrected[1]  # Correct qubit 1
...
def analyze_results(results):
    """Analyze phase-flip code results with post-processing error correction."""
    print("\n" + "=" * 60)
    print("3-QUBIT PHASE-FLIP CODE RESULTS")
    print("=" * 60)
    ...
    if error_qubit is not None:
        print(f"\n  Note: Without ANY correction, the phase error would corrupt the state")
    ...
    # Explanation
    print(f"\nHow phase-flip code works:")
    print("  Encoding: |0⟩ → |+++⟩, |1⟩ → |---⟩")
    print("  Syndrome measurement in X basis detects Z errors")
    print("  Syndrome 00: No error → no correction")
    print("  Syndrome 01: Z error on q2 → apply Z to q2")
    print("  Syndrome 10: Z error on q0 → apply Z to q0")
    print("  Syndrome 11: Z error on q1 → apply Z to q1")
...
if __name__ == "__main__":
    try:
        print("Executing 3-Qubit Phase-Flip Code on IBM Quantum hardware...")
        print("\nThe phase-flip code protects against single phase (Z) errors.")
        print("It encodes one logical qubit into three physical qubits in the X basis:")
        print("  |0⟩ → |+++⟩")
        print("  |1⟩ → |---⟩")
        print("\nThis is the bit-flip code transformed by Hadamard gates.\n")

        # Configuration
        initial_state = '1'  # Logical qubit state: '0' or '1'
        error_qubit = 1      # Apply Z error to qubit 0, 1, 2, or None for no error

        results = run_phase_flip_code(initial_state, error_qubit, shots=1024)
